...
class User(Model):
    ...
    streak_broken_at — UTC-момент локальной полуночи дня, СЛЕДУЮЩЕГО за первым
    пропущенным днём текущего эпизода потери. Именно с этого момента отсчитывается
    48-часовое окно восстановления (services.streaks.RESTORE_WINDOW_HOURS).
    Проставляется одновременно со streak_before_break и сбрасывается вместе с ним
    (restore/decline/новый выполненный день) — см. services/streaks.py.
    ...
    streak_broken_at = fields.DatetimeField(null=True)
