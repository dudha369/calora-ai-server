...
async def test_create_food_log_auto_logs_water(client: AsyncClient, seeded_user):
    ...
    resp = await client.post("/api/food/log", json={
        "log_date": "2026-06-12",
        "items": [{"food_name": "Кофе с молоком", "portion_g": 250, "calories": 40,
                    "protein_g": 1.5, "fat_g": 1.5, "carbs_g": 4.0}],
        "water_ml": 240,
    })
...
@pytest.mark.asyncio
async def test_create_food_log_without_water_skips_water_log(client: AsyncClient, seeded_user):
    """No water_ml (or 0/null) means no WaterLog gets created."""
    await client.post("/api/food/log", json={
        "log_date": "2026-06-12",
        "items": [{"food_name": "Рис", "portion_g": 150, "calories": 195,
                    "protein_g": 4.1, "fat_g": 0.5, "carbs_g": 43.0}],
    })
...
async def test_analyze_forwards_user_notes(mock_upload, mock_analyze, client: AsyncClient, seeded_user):
...
@patch("api.food.analyze_food_photo", new_callable=AsyncMock)
@patch("api.food.upload_food_photo", new_callable=AsyncMock)
async def test_analyze_without_notes_passes_none(mock_upload, mock_analyze, client: AsyncClient, seeded_user):
