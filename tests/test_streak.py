...
from services.streaks import reconcile_streak, credit_today_if_goal_met
...
async def test_credit_ignores_backdated_log(seeded_user):
    ...
    await _set_last_checked(YESTERDAY)
    await FoodLog.create(user_id=FAKE_TG_USER_ID, log_date=YESTERDAY, total_calories=2000)
...
@pytest.mark.asyncio
async def test_food_log_endpoint_credits_streak_end_to_end(client: AsyncClient, seeded_user):
